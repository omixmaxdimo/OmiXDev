...
class DealGame:
    ...
    def __init__(self):
        #self.deck=[]
        self.jokers = self.set_jokers()
        self.deck = self.create_deck(self.jokers)
        self.players = []
        self.table_hand = []
        self.bet_flop = 0
        self.bet_turn = 0
    ...
    def create_deck(self,joker_num):
        ...
        print(">>> Total card befor joker:",len(deck))
        if joker_num>0:
            print("!!!! Adding Jokers",joker_num)
            for i in range(joker_num):
                deck.append(Card(str(i),"JOKER"))
                print("JOKER ",i)
        random.shuffle(deck)
...
class DealRandom:
    ...
    def Deal(self,SEED,Game_Name,Suit_colors,Sort_Hand_method,Deal_Num,Deal_table_Num,Deal_Jokers_Num,Player_1_name,Player_2_name,Player_3_name,Player_4_name,Player_5_name,Player_6_name,Player_1_Sited,Player_2_Sited,Player_3_Sited,Player_4_Sited,Player_5_Sited,Player_6_Sited):
        ...
        game = DealGame()
        game.set_jokers(Deal_Jokers_Num)
        jk = Deal_Jokers_Num
        # Define Players
